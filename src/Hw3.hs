...
removeZero [] = []
removeZero (x:ds) = if (x == 0) then removeZero ds else x:ds 
...
bigAdd l1 l2     = removeZero res
  where
    ...
    f a x        = case (a,x) of 
		   ...
		   (a1:a', (l,r)) -> (l+r+a1) `div` 10 : (l+r+a1) `mod` 10 : a'
    base         = [0]
...
mulByDigit 0 n = [0]
mulByDigit i n = bigAdd n (mulByDigit (i-1) n)
...
bigMul l1 l2 = res
  where
    ...
    f (a,b) x    = (a+1, (bigAdd b (x ++ clone 0 a)))		
    base     = (0, [0])
